...
| 증상                                                                                        | 원인                                                | 조치                                                                                                                                                                                                |
| ------------------------------------------------------------------------------------------- | --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **공고 목록·상세가 404** ("존재하지 않는 공고" / "목록을 불러오지 못했습니다"). 채팅은 정상 | 백엔드에 `/rfps` 없음 — `feat/rfp-cards-api` 미머지 | 백엔드 브랜치 `feat/rfp-cards-api` 를 main에 머지. 확인: `curl -s -o /dev/null -w '%{http_code}' -H "X-API-Token: devtoken" http://localhost:8000/rfps` 가 `200` 이어야 함. 상세는 `CLAUDE.md` 참조 |
